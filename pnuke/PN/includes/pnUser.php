...
function pnUserLogIn($uname, $pass, $rememberme=false)
{
    ...
    if (!pnUserLoggedIn()) {
        ...
        $query = "update $ssitable where uid=" . pnVarPrepForStore($uid) . " set logon = '$pass64' ";
        $result =& $dbconn->Execute($query);
        if (!$result) {
			// user does not yet exist in SSI table
			$query = "insert into  $ssitable (uid,logon) values( $uid,'$pass64')";
			$result =& $dbconn->Execute($query);
        }
		//pnSSI

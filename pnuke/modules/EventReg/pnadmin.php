...
// Purpose of file:  EventReg administration display functions
// ----------------------------------------------------------------------
/*
************************************************************
TODO: 
...
{ 
    ...
    if (!pnSecAuthAction(0, 'EventReg::', '::', ACCESS_EDIT)) {
    ...
    $output->Text(eventreg_admin_viewevents());
 ...
 * view items
 */
function eventreg_admin_viewevents()
{ 
    ...
    // menu at their head to aid in navigation
    $output->SetInputMode(_PNH_VERBATIMINPUT);
    $output->Text(eventreg_adminmenu());
    $output->SetInputMode(_PNH_PARSEINPUT); 
    ...
    $items = pnModAPIFunc('EventReg',
        ...
        array('startnum' => $startnum,
            'numitems' => $itemsperpage)); //pnModGetVar('EventReg','itemsperpage')
	//End of pager
    // Start output table
    $output->TableStart('',
        array(_EVENTNAME,
    ...
    foreach ($items as $item) {
        ...
        if (pnSecAuthAction(0, 'EventReg::', ":$item[categoryid]:$item[eventid]", ACCESS_READ)) {
            ...
            $row[] = '&nbsp;' . $startdate;
			*/
			$row[] = '&nbsp;' . pnVarCensor(date(pnModGetVar('EventReg','dateformat') . ' ' . pnModGetVar('EventReg','timeformat'),$item['event_start']));
            ...
            if (pnSecAuthAction(0, 'EventReg::', "$item[name]::$item[eventid]", ACCESS_EDIT)) {
                $options[] = $output->URL(pnModURL('EventReg',
                        'admin',
                        'modifyevent',
                        array('eventid' => $item['eventid'])),
                    _EDIT);
                if (pnSecAuthAction(0, 'EventReg::', "$item[name]::$item[eventid]", ACCESS_DELETE)) {
                    $options[] = $output->URL(pnModURL('EventReg',
                            'admin',
                            'deleteevent',
                            array('eventid' => $item['eventid'])),
            ...
            $totalregs = pnModAPIFunc('EventReg', 'user', 'countregistrationsforevent',
                array('eventid' => $item['eventid']));
            $row[] = $output->URL(pnModURL('EventReg', 'admin',
                    'displayevent',
                    array('eventid' => $item['eventid'])),
                $totalregs);
								
								//Moved hidden form that passes data to PostCalendar into it's own function
	if (!pnModLoad('EventReg', 'admin')) {
    ...
    $output->SetInputMode(_PNH_VERBATIMINPUT);
	$row[] = $output -> Text(pnModFunc('EventReg',
        'admin',
        'PostCalendarForm',
        array('eventid' => $item['eventid'])));
	...
	//Got pager working 1-31-05 Brian M
  $total = pnModAPIFunc('EventReg', 'user', 'countitems'); //created the function
//echo $total;
    $output->Pager($startnum,
        ...
        pnModURL('EventReg',
            ...
            'viewevents',
            array('startnum' => '%%')),
        $itemsperpage); //pnModGetVar('EventReg', 'itemsperpage') <-need to get this mod var setup
		//end of pager
    // Return the output that has been generated by this function
    return $output->GetOutput();
} 

function eventreg_admin_displayevent($args)
{ 
    // Get parameters from whatever input we need.  All arguments to this
    // function should be obtained from pnVarCleanFromInput(), getting them
    // from other places such as the environment is not allowed, as that makes
...
function eventreg_admin_viewcategories()
{ 
    // Get parameters from whatever input we need.  All arguments to this
    // function should be obtained from pnVarCleanFromInput(), getting them
    // from other places such as the environment is not allowed, as that makes
    ...
    foreach ($items as $item) {
        ...
        if (pnSecAuthAction(0, 'EventReg::', ":$item[categoryid]:$item[eventid]", ACCESS_READ)) {
            ...
            $numevents = pnModAPIFunc('EventReg', 'user', 'counteventsincategory', array('categoryid' => $categoryid));
            $row[] = '&nbsp;' . $item['name'];
            $row[] = '&nbsp;' . $item['description'];
            $row[] = '&nbsp;' . $numevents; 
            // Options for the item.  Note that each item has the appropriate
            ...
            $options = array();
            $output->SetOutputMode(_PNH_RETURNOUTPUT);
            if (pnSecAuthAction(0, 'EventReg::', "$item[name]::$item[tid]", ACCESS_EDIT)) {
                $options[] = $output->URL(pnModURL('EventReg',
                        'admin',
                        'modifycategory',
                        array('categoryid' => $item['categoryid'])),
                    _EDIT);
                if (pnSecAuthAction(0, 'EventReg::', "$item[name]::$item[tid]", ACCESS_DELETE)) {
                    $options[] = $output->URL(pnModURL('EventReg',
                            'admin',
                            'deletecategory',
                            array('categoryid' => $item['categoryid'])),
...
function eventreg_admin_viewregistrations()
{ 
    // Get parameters from whatever input we need.  All arguments to this
    // function should be obtained from pnVarCleanFromInput(), getting them
    // from other places such as the environment is not allowed, as that makes
    ...
    if (!pnSecAuthAction(0, 'EventReg::registrations', '::', ACCESS_EDIT)) {
        $output->Text(_EVENTREGNOAUTH);
        return $output->GetOutput();
    } 
    // Add menu to output - it helps if all of the module pages have a standard
 ...
 * module
 */
function eventreg_admin_modifyconfig()
{ 
    // Create output object - this object will store all of our output so that
    // we can return it easily when required
    $output = new pnHTML(); 
...
*/
    ...
    $output->FormStart(pnModURL('EventReg', 'admin', 'updateconfig')); 
    ...
    $row[] = $output->FormSelectMultiple('mapservice',$maplist,0,1,pnModGetVar('EventReg', 'mapservice'));
    $output->SetOutputMode(_PNH_KEEPOUTPUT);
    $output->SetInputMode(_PNH_VERBATIMINPUT);
   ...
   foreach($ids as $k => $v) {
    ...
    $row[] = $output->FormSelectMultiple('viewregistrationlevel',$reglevels,0,1,pnModGetVar('EventReg', 'viewregistrationlevel'));
    $output->SetOutputMode(_PNH_KEEPOUTPUT);
    $output->SetInputMode(_PNH_VERBATIMINPUT);
    $output->TableAddrow($row, 'left');
    $output->SetInputMode(_PNH_PARSEINPUT);
    ...
    $row[] = $output->FormSelectMultiple('timeformat',$timeformats,0,1,pnModGetVar('EventReg', 'timeformat'));
    $output->SetOutputMode(_PNH_KEEPOUTPUT);
    $output->SetInputMode(_PNH_VERBATIMINPUT);
    $output->TableAddrow($row, 'left');
    $output->SetInputMode(_PNH_PARSEINPUT);
    ...
    $row[] = $output->FormSelectMultiple('dateformat',$dateformats,0,1,pnModGetVar('EventReg', 'dateformat'));
    $output->SetOutputMode(_PNH_KEEPOUTPUT);
    $output->SetInputMode(_PNH_VERBATIMINPUT);
    $output->TableAddrow($row, 'left');
    $output->SetInputMode(_PNH_PARSEINPUT);
    ...
    $row[] = $output->FormText('categorycolumns', pnModGetVar('EventReg', 'categorycolumns'), 3, 3);
    $output->SetOutputMode(_PNH_KEEPOUTPUT);
    ...
    $row[] = $output->FormText('eventsperpage', pnModGetVar('EventReg', 'eventsperpage'), 3, 3);
    $output->SetOutputMode(_PNH_KEEPOUTPUT);
    ...
    $row[] = $output->FormText('notifyemail', pnModGetVar('EventReg', 'notifyemail'),70);
    $output->SetOutputMode(_PNH_KEEPOUTPUT);
    $output->SetInputMode(_PNH_VERBATIMINPUT);
    $output->TableAddrow($row, 'left');
    $output->SetInputMode(_PNH_PARSEINPUT);
    ...
    $row[] = $output->FormCheckBox('multiregs', pnModGetVar('EventReg', 'AllowMultipleRegs'),70);
    $output->SetOutputMode(_PNH_KEEPOUTPUT);
    $output->SetInputMode(_PNH_VERBATIMINPUT);
    $output->TableAddrow($row, 'left');
    $output->SetInputMode(_PNH_PARSEINPUT);
 ...
 */
function eventreg_admin_updateconfig()
{ 
    // Get parameters from whatever input we need.  All arguments to this
    // function should be obtained from pnVarCleanFromInput(), getting them
    // from other places such as the environment is not allowed, as that makes
    ...
    if (!pnSecConfirmAuthKey()) {
        ...
        pnRedirect(pnModURL('EventReg', 'admin', 'view'));
        return true;
    } 
...
/*    if (!isset($bold)) {
    ...
    pnModSetVar('EventReg', 'mapservice', $mapservice);
 
 
 
 
 
    pnModSetVar('EventReg', 'viewregistrationlevel', $viewregistrationlevel); 
	pnModSetVar('EventReg','categorycolumns', $categorycolumns);
	pnModSetVar('EventReg','eventsperpage', $eventsperpage);
	pnModSetVar('EventReg','dateformat', $dateformat);
	pnModSetVar('EventReg','timeformat', $timeformat);
	pnModSetVar('EventReg','notifyemail', $notifyemail);
        pnModSetVar('EventReg', 'AllowMultipleRegs',$multiregs);
    // This function generated no output, and so now it is complete we redirect
    // the user to an appropriate page for them to carry on their work
    pnRedirect(pnModURL('EventReg', 'admin', 'modifyconfig')); 
    // Return
    return true;
} 
/**
...
{ 
    ...
    $columns2 = array();
    $columns2[] = $output->URL(pnModURL('EventReg',
            'admin',
            'viewcategories'),
        _VIEWCATEGORY);
    $columns2[] = $output->URL(pnModURL('EventReg',
 ...
 */
function eventreg_admin_newcategory()
{ 
    // Create output object - this object will store all of our output so that
    // we can return it easily when required
    $output = new pnHTML(); 
    ...
    // this form.  All URLs should be generated through pnModURL() to ensure
    // compatibility with future versions of PostNuke
    $output->FormStart(pnModURL('EventReg', 'admin', 'createcategory')); 
    // Add an authorisation ID - this adds a hidden field in the form that
    // contains an authorisation ID.  The authorisation ID is very important in
    // preventing certain attacks on the website
    $output->FormHidden('authid', pnSecGenAuthKey()); 
    ...
    $PostCalcategory = pnModAPIFunc('EventReg', 'admin', 'getpostcalendarcategories', array());
    $row = array();
    $output->SetOutputMode(_PNH_RETURNOUTPUT);
    if (!$PostCalcategory) {
 ...
 * form supplied by eventreg_admin_new() to create a new item
 * 
 * @param  $ 'name' the name of the item to be created
 * @param  $ 'number' the number of the item to be created
 */
function eventreg_admin_createcategory($args)
{ 
    // Get parameters from whatever input we need.  All arguments to this
    // function should be obtained from pnVarCleanFromInput(), getting them
    // from other places such as the environment is not allowed, as that makes
    ...
    if (!pnSecConfirmAuthKey()) {
        pnSessionSetVar('errormsg', _BADAUTHKEY);
        pnRedirect(pnModURL('EventReg', 'admin', 'error'));
        return true;
    } 
    ...
    $categoryid = pnModAPIFunc('EventReg',
        'admin',
        'createcategory',
        array('name' => $name,
    ...
    // This function generated no output, and so now it is complete we redirect
    // the user to an appropriate page for them to carry on their work
    pnRedirect(pnModURL('EventReg', 'admin', 'viewcategories')); 
    // Return
    return true;
} 
 ...
 */
function eventreg_admin_modifycategory($args)
{ 
    // Get parameters from whatever input we need.  All arguments to this
    // function should be obtained from pnVarCleanFromInput(), getting them
    // from other places such as the environment is not allowed, as that makes
    ...
    // do the check
    if (!pnSecAuthAction(0, 'EventReg::Item', "$item[name]::$eventid", ACCESS_EDIT)) {
    ...
    // this form.  All URLs should be generated through pnModURL() to ensure
    // compatibility with future versions of PostNuke
    $output->FormStart(pnModURL('EventReg', 'admin', 'updatecategory')); 
    // Add an authorisation ID - this adds a hidden field in the form that
    // contains an authorisation ID.  The authorisation ID is very important in
    // preventing certain attacks on the website
    $output->FormHidden('authid', pnSecGenAuthKey()); 
...
/**
 * This is a standard function that is called with the results of the
 * form supplied by eventreg_admin_modify() to update a current item
 * 
 ...
 */
function eventreg_admin_updatecategory($args)
{ 
    // Get parameters from whatever input we need.  All arguments to this
    // function should be obtained from pnVarCleanFromInput(), getting them
    // from other places such as the environment is not allowed, as that makes
 ...
 * wishes to create a new module item
 */
function eventreg_admin_newevent()
{ 
    // Create output object - this object will store all of our output so that
    // we can return it easily when required
    $ModName = basename(dirname(__FILE__));
    ...
    // this form.  All URLs should be generated through pnModURL() to ensure
    // compatibility with future versions of PostNuke
    $output->FormStart(pnModURL('EventReg', 'admin', 'createevent')); 
    // Add an authorisation ID - this adds a hidden field in the form that
    // contains an authorisation ID.  The authorisation ID is very important in
    // preventing certain attacks on the website
    $output->FormHidden('authid', pnSecGenAuthKey());
    ...
    if (!$EventRegcategory) {
        ...
        $row[] = $output->FormSelectMultiple('category', $EventRegcategory);
    } 
    $output->SetOutputMode(_PNH_KEEPOUTPUT);
    $output->SetInputMode(_PNH_VERBATIMINPUT);
    $output->TableAddrow($row, 'left');
    ...
    if ($useinternationaldates) {
        $sel_data = pnModAPIFunc('EventReg', 'user', 'buildDaySelect', array('pc_day' => $day, 'selected' => $event_startday));
        $row[2] = $output->FormSelectMultiple('regstartday', $sel_data);
        $sel_data = pnModAPIFunc('EventReg', 'user', 'buildMonthSelect', array('pc_month' => $month, 'selected' => $event_startmonth));
    ...
    $sel_data = pnModAPIFunc('EventReg', 'user', 'buildYearSelect', array('pc_year' => $year, 'selected' => $event_startyear));
    ...
    $output->SetOutputMode(_PNH_RETURNOUTPUT);
    $row[] = $output->Text(pnVarPrepForDisplay(_NOTIFYEMAIL));
    $row[] = $output->FormText('notifyemail', pnModGetVar('EventReg','notifyemail'));
    $output->SetOutputMode(_PNH_KEEPOUTPUT);
    $output->SetInputMode(_PNH_VERBATIMINPUT);
    $output->TableAddrow($row, 'left');
    $output->SetInputMode(_PNH_PARSEINPUT);
...
function eventreg_admin_createevent($args)
{ 
    // Get parameters from whatever input we need.  All arguments to this
    // function should be obtained from pnVarCleanFromInput(), getting them
    // from other places such as the environment is not allowed, as that makes
    ...
    if (!pnSecConfirmAuthKey()) {
        pnSessionSetVar('errormsg', _BADAUTHKEY);
        pnRedirect(pnModURL('EventReg', 'admin', 'newevent'));
        return true;
    } 
    ...
    if ($eventid != false) {
        ...
        $output->URL(pnModURL('EventReg',
        					'admin',
							'prepcalendarevent',
							array('eventid' => $eventid)),"Create PostCalendar Event");
...
*/
function eventreg_admin_PostCalendarForm($args) {
extract($args);
...
//Add a form with hidden fields to pass event info to PostCalendar
    ...
    $pc_event = pnModAPIFunc('EventReg',
        'admin',
    ...
    //Event Category (this is the PC category, not the EventReg category)
	...
	$output->FormHidden('event_conttel', $pc_event[event_conttel]); 
        //Event contact email address (use same as the notify email for EventReg)
...
function eventreg_admin_modifyevent($args)
{ 
    // Get parameters from whatever input we need.  All arguments to this
    // function should be obtained from pnVarCleanFromInput(), getting them
    // from other places such as the environment is not allowed, as that makes
    ...
    // compatibility with future versions of PostNuke
    $output->UploadMode();
    $output->FormStart(pnModURL('EventReg', 'admin', 'updateevent')); 
    // Add an authorisation ID - this adds a hidden field in the form that
    // contains an authorisation ID.  The authorisation ID is very important in
    // preventing certain attacks on the website
    $output->FormHidden('authid', pnSecGenAuthKey()); 
    ...
    // the update function so that it knows which item for which item to carry
    // out the update
    $output->FormHidden('eventid', pnVarPrepForDisplay($eventid)); 
    // Start the table that holds the information to be input.  Note how each
    // item in the form is kept logically separate in the code; this helps to
    // see which part of the code is responsible for the display of each item,
    // and helps with future modifications
    ...
    $output->TableAddrow($row, 'left');
    $output->SetInputMode(_PNH_PARSEINPUT);
    $output->FormStart(pnModURL('EventReg', 'admin', 'createevent')); // LDH - WTF? This is extra and  unbalanced?!?
    // Category selection
    // Load API.  All of the actual work for the creation of the new item is
    // done within the API, so we need to load that in before we can do
    // anything.  If the API fails to load an appropriate error message is
    ...
    if (!$EventRegcategory) {
        ...
        $row[] = $output->Text(pnVarPrepForDisplay(_SELECTEVENTCATEGORY));
        $row[] = $output->FormSelectMultiple('category', $EventRegcategory, 0, 1, $category);
    } 
    $output->SetOutputMode(_PNH_KEEPOUTPUT);
    $output->SetInputMode(_PNH_VERBATIMINPUT);
    $output->TableAddrow($row, 'left');
    $output->SetInputMode(_PNH_PARSEINPUT);
    // Registration Start Date/time
    $reg_starttimes = pnModAPIFunc('EventReg', 'user', 'buildTimeSelect', array('hselected' => date('g',$event[reg_start]), 'mselected' => date('i',$event[reg_start])));
    $row = array();
    $output->SetOutputMode(_PNH_RETURNOUTPUT);
    $row[] = $output->Text(pnVarPrepForDisplay(_REGSTARTDATETM));
    $sel_data = pnModAPIFunc('EventReg', 'user', 'buildMonthSelect', array('pc_month' => $month, 'selected' => date('m',$reg_start)));
    $row[2] = $output->FormSelectMultiple('regstartmonth', $sel_data);
    $sel_data = pnModAPIFunc('EventReg', 'user', 'buildDaySelect', array('pc_day' => $day, 'selected' => date('d',$event[reg_start])));
    $row[2] .= $output->FormSelectMultiple('regstartday', $sel_data);
    $sel_data = pnModAPIFunc('EventReg', 'user', 'buildYearSelect', array('pc_year' => $year, 'selected' => date('Y',$event[reg_start])));
    $row[2] .= $output->FormSelectMultiple('regstartyear', $sel_data);
    $row[2] .= $output->Text('  ');
    ...
    $output->SetInputMode(_PNH_PARSEINPUT); 
    // Registration End Date/time
    $reg_endtimes = pnModAPIFunc('EventReg', 'user', 'buildTimeSelect', array('hselected' => date('g',$event[reg_end]), 'mselected' => date('i',$event[reg_end])));

    $row = array();
    $output->SetOutputMode(_PNH_RETURNOUTPUT);
    $row[] = $output->Text(pnVarPrepForDisplay(_REGENDDATETM));
        $sel_data = pnModAPIFunc('EventReg', 'user', 'buildMonthSelect', array('pc_month' => $month, 'selected' => date('m',$event[reg_end])));
        $row[2] = $output->FormSelectMultiple('regendmonth', $sel_data);
        $sel_data = pnModAPIFunc('EventReg', 'user', 'buildDaySelect', array('pc_day' => $day, 'selected' => date('d',$event[reg_end])));
        $row[2] .= $output->FormSelectMultiple('regendday', $sel_data);
    $sel_data = pnModAPIFunc('EventReg', 'user', 'buildYearSelect', array('pc_year' => $year, 'selected' => date('Y',$event[reg_end])));
    $row[2] .= $output->FormSelectMultiple('regendyear', $sel_data);
    $row[2] .= $output->Text('  ');
    ...
    $output->SetInputMode(_PNH_PARSEINPUT); 
    // event Start Date/time
    $event_starttimes = pnModAPIFunc('EventReg', 'user', 'buildTimeSelect', array('hselected' => date('g',$event[event_start]), 'mselected' => date('i',$event[event_start])));
    $row = array();
    $output->SetOutputMode(_PNH_RETURNOUTPUT);
    $row[] = $output->Text(pnVarPrepForDisplay(_EVENTSTARTDATETM));
        $sel_data = pnModAPIFunc('EventReg', 'user', 'buildMonthSelect', array('pc_month' => $month, 'selected' => date('m',$event[event_start])));
        $row[2] = $output->FormSelectMultiple('eventstartmonth', $sel_data);
        $sel_data = pnModAPIFunc('EventReg', 'user', 'buildDaySelect', array('pc_day' => $day, 'selected' => date('d',$event[event_start])));
        $row[2] .= $output->FormSelectMultiple('eventstartday', $sel_data);
    $sel_data = pnModAPIFunc('EventReg', 'user', 'buildYearSelect', array('pc_year' => $year, 'selected' => date('Y',$event[event_start])));
    $row[2] .= $output->FormSelectMultiple('eventstartyear', $sel_data);
    $row[2] .= $output->Text('  ');
    ...
    $output->SetInputMode(_PNH_PARSEINPUT); 
    // event end Start Date/time
    $event_endtimes = pnModAPIFunc('EventReg', 'user', 'buildTimeSelect', array('hselected' => date('g',$event[event_end]), 'mselected' => date('i',$event[event_end])));
    $row = array();
    $output->SetOutputMode(_PNH_RETURNOUTPUT);
    $row[] = $output->Text(pnVarPrepForDisplay(_EVENTENDDATETM));
        $sel_data = pnModAPIFunc('EventReg', 'user', 'buildMonthSelect', array('pc_month' => $month, 'selected' => date('m',$event[event_end])));
        $row[2] = $output->FormSelectMultiple('eventendmonth', $sel_data);
        $sel_data = pnModAPIFunc('EventReg', 'user', 'buildDaySelect', array('pc_day' => $day, 'selected' => date('d',$event[event_end])));
        $row[2] .= $output->FormSelectMultiple('eventendday', $sel_data);
    $sel_data = pnModAPIFunc('EventReg', 'user', 'buildYearSelect', array('pc_year' => $year, 'selected' => date('Y',$event[event_end])));
    $row[2] .= $output->FormSelectMultiple('eventendyear', $sel_data);
    $row[2] .= $output->Text('  ');
    ...
    $output->SetOutputMode(_PNH_RETURNOUTPUT);
    $row[] = $output->Text(pnVarPrepForDisplay(_EVENTMAXREGISTRATIONS));
    $row[] = $output->FormText('maxregistrations', $event[max_regs], 9, 9);
    $output->SetOutputMode(_PNH_KEEPOUTPUT);
    $output->SetInputMode(_PNH_VERBATIMINPUT);
    $output->TableAddrow($row, 'left');
    $output->SetInputMode(_PNH_PARSEINPUT);
 ...
 * @param  $ 'number' the number of the item to be updated
 */
function eventreg_admin_updateevent($args)
{
       if (!pnSecConfirmAuthKey()) {
        pnSessionSetVar('errormsg', _BADAUTHKEY);
        pnRedirect(pnModURL('EventReg', 'admin', 'newevent'));
...
function eventreg_admin_deleteevent($args)
{ 
    // Get parameters from whatever input we need.  All arguments to this
    // function should be obtained from pnVarCleanFromInput(), getting them
    // from other places such as the environment is not allowed, as that makes
    ...
    list($eventid,
        $objectid,
        $confirmation) = pnVarCleanFromInput('eventid',
        'objectid',
        'confirmation'); 
    // User functions of this type can be called by other modules.  If this
    // happens then the calling module will be able to pass in arguments to
    ...
    // complete the instance information so this is the first chance we get to
    // do the check
    if (!pnSecAuthAction(0, 'EventReg::event', "$event[addedby]:$event[category]:$event[eventid]", ACCESS_DELETE)) {
        $output->Text(_EventRegNOAUTH);
        return $output->GetOutput();
    } 
    ...
    if (!pnSecConfirmAuthKey()) {
        pnSessionSetVar('errormsg', _BADAUTHKEY);
        pnRedirect(pnModURL('EventReg', 'admin', 'viewevents'));
        return true;
    } 
...
function eventreg_admin_deletecategory($args)
{ 
    // Get parameters from whatever input we need.  All arguments to this
    // function should be obtained from pnVarCleanFromInput(), getting them
    // from other places such as the environment is not allowed, as that makes
    ...
    // complete the instance information so this is the first chance we get to
    // do the check
    if (!pnSecAuthAction(0, 'EventReg::category', "$category[name]::$eventid", ACCESS_DELETE)) {
        $output->Text(_EventRegNOAUTH);
        return $output->GetOutput();
    } 
    // Check for confirmation.
...
function eventreg_admin_prepcalendarevent($args) {

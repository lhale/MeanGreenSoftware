...
// Filename: includes/search/EventReg.php
// Original Author: Erik Bartels
// Purpose: Search EventReg
$search_modules[] = array(
    'title' => 'Event Registration',
    'func_search' => 'search_eventreg',
    'func_opt' => 'search_eventreg_opt'
);

function search_eventreg_opt() {
    global
        $bgcolor2,
        $textcolor1;
...
function search_eventreg() {
    ...
    $eventcolumn = pnConfigGetVar('prefix') . '_eventreg_events';
    $catcolumn = pnConfigGetVar('prefix') . '_eventreg_categories';
	$query = "SELECT " . 
				$eventcolumn . ".pn_id as eventid, " . 
    ...
    if (empty($total)) {
		...
		while(!$countres->EOF) {
            ...
            if (pnSecAuthAction(0, 'eventreg::event', "::$row[eventid]", ACCESS_READ)) {
				$total++;
			}
			$countres->MoveNext();
		}

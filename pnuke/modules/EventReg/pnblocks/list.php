...
// Adapted for EventReg by Eric Mathieu
// ----------------------------------------------------------------------
...
function EventReg_listblock_display($blockinfo)
{
   // Security check
...
#    $tid = $vars['tid'];
#    if (!isset($tid))
#        $tid = pnModGetVar('EventReg','frontpagePubType');
#   if (!isset($tid))
#   {
...
#     $filters = preg_split("/\s*&\s*/", $filterStr);
   ...
   if (!pnModAPILoad('EventReg', 'user'))
     return EventRegErrorPage(__FILE__, __LINE__, 'Failed to load EventReg user API');
...
# 
   ...
   if ($smarty->template_exists($templateFile)) {
      ...
      foreach ($items as $item) {
         ...
         $item['fullURL'] = pnModURL('EventReg',
                                     'user',
                                     'displayevent',
                                     array('eventid' => $item['eventid']));
 ...
 */
function EventReg_listblock_modify($blockinfo)
{
  $output = new pnHTML();
...
#     // Defaults
#   if (!isset($vars['tid']))
#     $vars['tid'] = pnModGetVar('EventReg','frontpagePubType');
...
#     return EventRegErrorPage(__FILE__, __LINE__, 'Failed to load EventReg admin API');
# 
#     // (no table start/end since the block framework takes care of that)
# 
#     // Create row for "Publication type"
#   $pubTypesData = pnModAPIFunc('EventReg',
#                                'admin',
#                                'getPublicationTypes');
# 
#   $pubTypes = array();
 ...
 */
function EventReg_listblock_update($blockinfo)
{

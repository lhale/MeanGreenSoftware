 ...
 *
 *   $Id: groupcp.php 192 2007-01-20 15:17:44Z kronos $
 *
 *
 ***************************************************************************/
...
//	$temp_url = append_sid("privmsg.$phpEx?mode=post&amp;" . POST_USERS_URL . "=" . $row['user_id']);
	...
	{
	 	 $temp_url = "index.php?module=Messages&amp;func=compose&amp;uname=" . urlencode($row['username']);
	}
	else
	{	
		 $temp_url = append_sid("privmsg.$phpEx?mode=post&amp;" . POST_USERS_URL . "=" . $row['user_id']);
...
/*		'U_GROUPCP' => $server_url . '?' . POST_GROUPS_URL . "=$group_id&validate=true") */
		'U_GROUPCP' => $board_config['server_name'] . $board_config['module_url'] . "groupcp&" . POST_GROUPS_URL . "=$group_id&validate=true")
/* End PNphpBB2 Module */
	);
	$emailer->send();
	$emailer->reset();
...
//					WHERE username = '" . str_replace("\'", "''", $username) . "'";
				$sql = "SELECT user_id, user_email, user_lang, user_level  
					FROM " . USERS_TABLE . " 
					WHERE username = '" . pnVarPrepForStore($username) . "'";
// End PNphpBB2 Module
				if ( !($result = $db->sql_query($sql)) )
				{
...
/*						'U_GROUPCP' => $server_url . '?' . POST_GROUPS_URL . "=$group_id") */
						'U_GROUPCP' => $board_config['server_name'] . $board_config['module_url'] . "groupcp&" . POST_GROUPS_URL . "=$group_id")
/* End PNphpBB2 Module */
					);
					$emailer->send();
					$emailer->reset();
...
/*		'MOD_USERNAME' => $username, */
		'MOD_ROW_COLOR' => '#' . $phpbb_theme['td_color1'],
		'MOD_ROW_CLASS' => $phpbb_theme['td_class1'],
		'MOD_USERNAME' => pnVarPrepForDisplay($username),
/* End PNphpBB2 Module */
		'MOD_FROM' => $from,
		'MOD_JOINED' => $joined,
		'MOD_POSTS' => $posts,
...
*/
// -- Add --
	generate_pagination("groupcp.$phpEx?" . POST_GROUPS_URL . "=$group_id", $members_count, $board_config['topics_per_page'], $start);
// End PNphpBB2 Module
...
// Begin PNphpBB2 Module
		$s_hidden_fields .= '<input type="hidden" name ="op" value="modload">';
		$s_hidden_fields .= '<input type="hidden" name ="name" value="' . $ModName . '">';
		$s_hidden_fields .=	'<input type="hidden" name ="file" value="groupcp">';
// End PNphpBB2 Module

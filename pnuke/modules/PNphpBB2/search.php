 ...
 *
 *   $Id: search.php 192 2007-01-20 15:17:44Z kronos $
 *
 *
 ***************************************************************************/
...
//					WHERE username LIKE '" . str_replace("\'", "''", $search_author) . "'";
		    ...
		    $sql = 'SELECT user_id 
			    FROM ' . USERS_TABLE . "
			    WHERE username $sql_where '" . pnVarPrepForStore(preg_replace('#\*+#', '%', $search_author)). "'";
// End PNphpBB2 Module
...
//					message_die(GENERAL_ERROR, "Couldn't obtain list of matching users (searching for: $search_author)", "", __LINE__, __FILE__, $sql);
					message_die(GENERAL_ERROR, "Couldn't obtain list of matching users (searching for: " . pnVarPrepForDisplay($search_author) . ")", "", __LINE__, __FILE__, $sql);
// End PNphpBB2 Module
				}
...
//							$where_sql .= " AND u.user_id = p.poster_id AND u.username LIKE '$search_author' ";
							$where_sql .= " AND u.user_id = p.poster_id AND u.username LIKE '" . pnVarPrepForStore($search_author) . "'";
// End PNphpBB2 Module
						}
...
//											$hold_string = str_replace('<!-- #sh -->', '<span style="color:#' . $theme['fontcolor3'] . '"><b>', $hold_string);
											$hold_string = str_replace('<!-- #sh -->', '<span style="color:#' . $phpbb_theme['fontcolor3'] . '"><b>', $hold_string);
// End PNphpBB2 Module
										}
...
//								$message = preg_replace($highlight_match, '<span style="color:#' . $theme['fontcolor3'] . '"><b>\1</b></span>', $message);
								$message = preg_replace($highlight_match, '<span style="color:#' . $phpbb_theme['fontcolor3'] . '"><b>\1</b></span>', $message);
// End PNphpBB2 Module
							}
						}
					}
...
//				$poster .= ( $searchset[$i]['user_id'] != ANONYMOUS ) ? $searchset[$i]['username'] : ( ( $searchset[$i]['post_username'] != "" ) ? $searchset[$i]['post_username'] : $lang['Guest'] );
				$poster .= ( $searchset[$i]['user_id'] != ANONYMOUS ) ? pnVarPrepForDisplay($searchset[$i]['username']) : ( ( $searchset[$i]['post_username'] != "" ) ? pnVarPrepForDisplay($searchset[$i]['post_username']) : $lang['Guest'] );
// End PNphpBB2 Module
...
//				$topic_author .= ( $searchset[$i]['user_id'] != ANONYMOUS ) ? $searchset[$i]['username'] : ( ( $searchset[$i]['post_username'] != '' ) ? $searchset[$i]['post_username'] : $lang['Guest'] );
				$topic_author .= ( $searchset[$i]['user_id'] != ANONYMOUS ) ? pnVarPrepForDisplay($searchset[$i]['username']) : ( ( $searchset[$i]['post_username'] != '' ) ? pnVarPrepForDisplay($searchset[$i]['post_username']) : $lang['Guest'] );
// End PNphpBB2 Module
...
//				$last_post_author = ( $searchset[$i]['id2'] == ANONYMOUS ) ? ( ($searchset[$i]['post_username2'] != '' ) ? $searchset[$i]['post_username2'] . ' ' : $lang['Guest'] . ' ' ) : '<a href="' . append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . '='  . $searchset[$i]['id2']) . '">' . $searchset[$i]['user2'] . '</a>';
				$last_post_author = ( $searchset[$i]['id2'] == ANONYMOUS ) ? ( ($searchset[$i]['post_username2'] != '' ) ? pnVarPrepForDisplay($searchset[$i]['post_username2']) . ' ' : $lang['Guest'] . ' ' ) : '<a href="' . append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . '='  . $searchset[$i]['id2']) . '">' . pnVarPrepForDisplay($searchset[$i]['user2']) . '</a>';
// End PNphpBB2 Module

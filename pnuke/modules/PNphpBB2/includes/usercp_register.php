 ...
 *
 *   $Id: usercp_register.php 192 2007-01-20 15:17:44Z kronos $
 *
 *
 ***************************************************************************/
...
// Begin PNphpBB2 Module
// Removed because it was not needed added as part of the 2.0.6 update	
/*
...
// Begin PNphpBB2 Module
if (isset($HTTP_POST_VARS['changeprofile']))
{
	// Postnuke change user info
	pnredirect("user.php?op=edituser");
}

if (isset($HTTP_POST_VARS['refreshprofile']))
{
	$user_id = intval($HTTP_POST_VARS['user_id']);
...
if (isset($HTTP_POST_VARS['getpnavatar']))
{
	$user_id = intval($HTTP_POST_VARS['user_id']);
...
//				WHERE user_id = $user_id";
			$sql = "UPDATE " . USERS_TABLE . " 
				SET user_sig = '" . str_replace("\'", "''", $signature) . "', user_sig_bbcode_uid = '$signature_bbcode_uid', user_viewemail = $viewemail, user_attachsig = $attachsig, user_allowsmile = $allowsmilies, user_allowhtml = $allowhtml, user_allowbbcode = $allowbbcode, user_allow_viewonline = $allowviewonline, user_notify = $notifyreply, user_notify_pm = $notifypm, user_popup_pm = $popup_pm, user_dateformat = '" . str_replace("\'", "''", $user_dateformat) . "', user_lang = '" . str_replace("\'", "''", $user_lang) . "', user_style = $user_style, user_active = $user_active, user_actkey = '" . str_replace("\'", "''", $user_actkey) . "'" . $avatar_sql . " 
				WHERE user_id = $user_id";
// End PNphpBB2 Module
			if ( !($result = $db->sql_query($sql)) )
			{
...
/* Begin PNphpBB2 Module */
		'P_CHANGE_PROFILE' => $lang['PNphpBB2_Change_Profile'],
		'P_REFRESH_PROFILE' => $lang['PNphpBB2_Refresh_Profile'],
		'P_PROFILE_INFO' => $lang['PNphpBB2_Profile_Info'],
		'P_AVATAR_RETRIEVE' => ($lang['PNphpBB2_Get_PN_Avatar']) ? $lang['PNphpBB2_Get_PN_Avatar'] : "PostNuke",
/* End PNphpBB2 Module */		

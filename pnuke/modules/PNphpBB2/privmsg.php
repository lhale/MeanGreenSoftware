 ...
 *
 *   $Id: privmsg.php 192 2007-01-20 15:17:44Z kronos $
 *
 *
 ***************************************************************************/
...
/*		'MESSAGE_FROM' => $username_from, */
		'MESSAGE_TO' => pnVarPrepForDisplay($username_to),
		'MESSAGE_FROM' => pnVarPrepForDisplay($username_from),
/* End PNphpBB2 Module */
...
//					AND user_id <> " . ANONYMOUS;
			$sql = "SELECT user_id, user_notify_pm, user_email, user_lang, user_active 
				FROM " . USERS_TABLE . "
				WHERE username = '" . pnVarPrepForStore($to_username) . "'
					AND user_id <> " . ANONYMOUS;
// End PNphpBB2 Module
			if ( !($result = $db->sql_query($sql)) )
			{
...
/*	 			 	'USERNAME' => stripslashes($to_username), */
					'USERNAME' => pnVarPrepForDisplay($to_username), 
/* End PNphpBB2 Module */
...
/*					'U_INBOX' => $server_protocol . $server_name . $server_port . $script_name . '?folder=inbox') */
					'U_INBOX' => $board_config['server_name'] . $board_config['module_url'] . "privmsg&folder=inbox")
/* End PNphpBB2 Module */
				);
...
/*			'MESSAGE_FROM' => $userdata['username'],  */
			'MESSAGE_TO' => pnVarPrepForDisplay($to_username), 
			'MESSAGE_FROM' => pnVarPrepForDisplay($userdata['username']), 
/* End PNphpBB2 Module */
...
/*		'USERNAME' => $to_username, */
		'USERNAME' => pnVarPrepForDisplay(preg_replace($html_entities_match, $html_entities_replace, $to_username)),
/* End PNphpBB2 Module */
...
//		$msg_username = $row['username'];
		$msg_username = pnVarPrepForDisplay($row['username']);
// End PNphpBB2 Module
...
//			$msg_username = '<b>' . $msg_username . '</b>';
			$msg_username = '<b>' . pnVarPrepForDisplay($msg_username) . '</b>';
// End PNphpBB2 Module
		}
...
// End PNphpBB2 Module
		...
		$template->assign_block_vars('listrow', array(
			...
			'DATE' => $msg_date,
/* -- mod : File Attachment Mod v2 Version 2.4.3 ---------------------------------------------------- */
			'PRIVMSG_ATTACHMENTS_IMG' => (!intval($attach_config['disable_mod'])) ? privmsgs_attachment_image($privmsg_id) : '',
/* -- fin : File Attachment Mod v2 Version 2.4.3 ---------------------------------------------------- */
...
*/
// -- Add --
  generate_pagination("privmsg.$phpEx?folder=$folder", $pm_total, $board_config['topics_per_page'], $start);
}
else
{
	$template->assign_vars(array(

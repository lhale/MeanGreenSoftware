...
$support_status = ($HTTP_GET_VARS['support_status']) ? intval($HTTP_GET_VARS['support_status']) : intval($HTTP_POST_VARS['support_status']);

if ($ssu >= 0  && $support_status <> $ssu )
{
  ...
  $sql = "UPDATE " . TOPICS_TABLE . " 
	...
	if ( !$db->sql_query($sql) )
	{
	  message_die(GENERAL_ERROR, 'Could not update support status', '', __LINE__, __FILE__, $sql);
	}
}
...
// Begin PNphpBB2 Categories Hierarchie & Topic-Post Sort order & support status
// $sql = "SELECT t.topic_id, t.topic_title, t.topic_status, t.topic_replies, t.topic_time, t.topic_type, t.topic_vote, t.topic_last_post_id, f.forum_name, f.forum_status, f.forum_id, f.auth_view, f.auth_read, f.auth_post, f.auth_reply, f.auth_edit, f.auth_delete, f.auth_sticky, f.auth_announce, f.auth_pollcreate, f.auth_vote, f.auth_attachments" . $count_sql . "
//	FROM " . TOPICS_TABLE . " t, " . FORUMS_TABLE . " f" . $join_sql_table . " 
//	WHERE $join_sql
//		AND f.forum_id = t.forum_id
//		$order_sql";
$sql = "SELECT t.topic_id, t.topic_title, t.topic_status, t.topic_replies, t.topic_time, t.topic_type, t.topic_vote, t.topic_last_post_id, t.support_status, f.forum_name, f.forum_status, f.forum_id, f.auth_view, f.auth_read, f.auth_post, f.auth_reply, f.auth_edit, f.auth_delete, f.auth_sticky, f.auth_announce, f.auth_pollcreate, f.auth_vote, f.auth_attachments, f.cat_id, f.post_sort_order, f.support_forum, c.cat_title" . $count_sql . "
	FROM " . TOPICS_TABLE . " t, " . FORUMS_TABLE . " f," . CATEGORIES_TABLE . " c" . $join_sql_table . " 
	WHERE $join_sql
		AND f.forum_id = t.forum_id
		AND c.cat_id = f.cat_id
		$order_sql";
// End PNphpBB2 Categories Hierarchie Mod & Topic-Post Sort order & support status
...
// Begin PNphpBB2 Module (Topic/Post sort order) 06-11-2003
$post_sort_order = $forum_topic_data['post_sort_order'];
...
else
{
	$post_order = 'asc';
	$post_time_order = 'ASC';
// Begin PNphpBB2 Module Topic-Post sort order Mod
	if ( $post_sort_order == 0 )
	{
		if ($board_config['pnphpbb2_post_order'] == 1)
		{
			$post_order = 'dec';
...
*/
	$s_auth_can .= sprintf($lang['Rules_moderate'], '<a href="' . append_sid("modcp.$phpEx?" . POST_FORUM_URL . "=$forum_id&amp;sid=" . $userdata['session_id']) . '">', '</a>');
	$topic_mod .= '<a href="' . append_sid("modcp.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;mode=delete&amp;sid=". $userdata['session_id']) . '"><img src="' . $images['topic_mod_delete'] . '" alt="' . $lang['Delete_topic'] . '" title="' . $lang['Delete_topic'] . '" border="0" /></a>&nbsp;';
	$topic_mod .= '<a href="' . append_sid("modcp.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;mode=move&amp;sid=" . $userdata['session_id']). '"><img src="' . $images['topic_mod_move'] . '" alt="' . $lang['Move_topic'] . '" title="' . $lang['Move_topic'] . '" border="0" /></a>&nbsp;';
	$topic_mod .= ( $forum_topic_data['topic_status'] == TOPIC_UNLOCKED ) ? '<a href="' . append_sid("modcp.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;mode=lock&amp;sid=" . $userdata['session_id']) . '"><img src="' . $images['topic_mod_lock'] . '" alt="' . $lang['Lock_topic'] . '" title="' . $lang['Lock_topic'] . '" border="0" /></a>&nbsp;' : '<a href="' . append_sid("modcp.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;mode=unlock&amp;sid=". $userdata['session_id']) . '"><img src="' . $images['topic_mod_unlock'] . '" alt="' . $lang['Unlock_topic'] . '" title="' . $lang['Unlock_topic'] . '" border="0" /></a>&nbsp;';
	$topic_mod .= '<a href="' . append_sid("modcp.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;mode=split&amp;sid=" . $userdata['session_id']) . '"><img src="' . $images['topic_mod_split'] . '" alt="' . $lang['Split_topic'] . '" title="' . $lang['Split_topic'] . '" border="0" /></a>&nbsp;';
	$topic_mod .= '<a href="' . append_sid("modcp.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;mode=merge&amp;sid=" . $userdata['session_id']) . '"><img src="' . $images['topic_merge'] . '" alt="' . $lang['Merge_topics'] . '" title="' . $lang['Merge_topics'] . '" border="0" /></a>&nbsp;';
	$topic_mod .= '<a href="' . append_sid("modcp.$phpEx?" . POST_FORUM_URL . "=$forum_id&amp;sid=" . $userdata['session_id']) . '"><img src="' . $images['topic_moderate'] . '" alt="' . $lang['Moderate'] . '" title="' . $lang['Moderate'] . '" border="0" /></a>';
// End PNphpBB2 Module
}
...
{  
  ...
  if ( $is_auth['auth_mod'] )
  {
    ...
    { 
	    ...
	    $support_status .= '<option style="color:' . $lang['support_color'][$i] . '; background-color: #FAFAFA" value="' . $i . '"' . $selected . '>' . $lang['support_status'][$i] . '</option>';
    }
    $support_status .= '</select>&nbsp;&nbsp;<input type="submit" value="' .  $lang['Update'] . '" class="catbutton" name="submit" /></td>';
  }
  else
  {
    $template->assign_block_vars('display_support_status', array());
    $support_status = $lang['support_status'][$forum_topic_data['support_status']];
  }
}
// End PNphpBB2 Module (Support Status Mod)
...
/* Begin PNphpBB2 Module (Support Status Mod) */
	...
	'L_STATUS' => '<font style="font-size:120%;font-weight:bold;">' . $lang['Status'] . '</font>',
	...
	'L_CURRENT_STATUS' => '<font style="font-size:120%;font-weight:bold;color:' . $lang['support_color'][$forum_topic_data['support_status']] . ';">' . $lang['support_status'][$forum_topic_data['support_status']] . '</font></b>',
/* End PNphpBB2 Module (Support Status Mod) */

/* Begin PNphpBB2 Module ForumImages vt-slices */
	'VIEW_PREVIOUS_IMG' => $images['previous_topic'],
	'VIEW_NEXT_IMG' => $images['next_topic'],
	'EMAIL_TOPIC_IMG' => $images['email_topic'],
	'PRINT_TOPIC_IMG' => $images['print_topic'],
/*	'S_WATCH_TOPIC_IMG' => $s_watching_topic_img, */
/*	'U_WATCH_TOPIC' => $u_watching_topic, */
/*	'L_WATCH_TOPIC' => $l_watching_topic, */
/*	'WATCH_TOPIC' => $watching_topic, */
/* End PNphpBB2 Module */

/* Begin PNphpBB2 Module (Printable page mod) */
...
/* End PNphpBB2 Module (Printable page mod) */
...
// Begin PNphpBB2 Module (Email a friend)
if( $userdata['session_logged_in'])
{
	$tell_friend = append_sid("tellafriend") . '&topic=' . $topic_title . '&link=' . POST_TOPIC_URL . "=$topic_id";
	$template->assign_vars(array(
		'S_TELL_FRIEND' => '<a href="' . $tell_friend . '">' . $lang['Tell_Friend'] . '</a>',
		'S_TELL_FRIEND_IMG' => '<a href="' . $tell_friend . '"><img src="' . $images['email_topic'] . '" alt="' . $lang['Tell_Friend'] . '" title="' . $lang['Tell_Friend'] . '" border="0" /></a>',
		'S_WATCH_TOPIC' => $s_watching_topic,
		'S_WATCH_TOPIC_IMG' => $s_watching_topic_img));
}
// End PNphpBB2 Module (Email a friend)

// Begin PNphpBB2 Module View topic navigation bar
// Graphical bar
$template->set_filenames(array('navbarbody' => 'viewtopic_navbar_imgs.tpl'));
$template->assign_var_from_handle('NAVBAR', 'navbarbody');

// Text top
$template->set_filenames(array('navbartexttopbody' => 'viewtopic_navbar_text_top.tpl'));
$template->assign_var_from_handle('NAVBARTEXTTOP', 'navbartexttopbody');

// Bottom
$template->set_filenames(array('navbartextbotbody' => 'viewtopic_navbar_text_bot.tpl'));
$template->assign_var_from_handle('NAVBARTEXTBOT', 'navbartextbotbody');
// End PNphpBB2 Module
...
// Begin PNphpBB2 Module (Obtain forum level moderator status)
$forum_mods = forum_mod_status ($forum_id);
// End PNphpBB2 Module (Obtain forum level moderator status)
...
//	$poster = ( $poster_id == ANONYMOUS ) ? $lang['Guest'] : $postrow[$i]['username'];
	$poster	= ( $poster_id == ANONYMOUS ) ? ( ( !empty($postrow[$i]['post_username'] ) ) ? $postrow[$i]['post_username'] : $lang['Guest']) : $postrow[$i]['username'];
	$poster_profile = ( $poster_id == ANONYMOUS || !$userdata['session_logged_in'] ) ? pnVarPrepForDisplay($poster) : '<a href="' . append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . '=' . $poster_id) . '">' . pnVarPrepForDisplay($poster) . '</a>';  
// End PNphpBB2 Module
...
//		$temp_url = "modcp.$phpEx?mode=ip&amp;" . POST_POST_URL . "=" . $postrow[$i]['post_id'] . "&amp;" . POST_TOPIC_URL . "=" . $topic_id . "&amp;sid=" . $userdata['session_id'];
		$temp_url = append_sid("modcp.$phpEx?mode=ip&amp;" . POST_POST_URL . "=" . $postrow[$i]['post_id'] . "&amp;" . POST_TOPIC_URL . "=" . $topic_id . "&amp;sid=" . $userdata['session_id']);
// End PNphpBB2 Module
...
//		$temp_url = "posting.$phpEx?mode=delete&amp;" . POST_POST_URL . "=" . $postrow[$i]['post_id'] . "&amp;sid=" . $userdata['session_id'];
		$temp_url = append_sid("posting.$phpEx?mode=delete&amp;" . POST_POST_URL . "=" . $postrow[$i]['post_id'] . "&amp;sid=" . $userdata['session_id']);
// End PNphpBB2 Module
...
// Begin PNphpBB2 Module (Support for Autolinks)
	list($message) = pnModCallHooks('item', 'transform', '', array($message));
// End PNphpBB2 Module (Support for Autolinks)

	$template->assign_block_vars('postrow', array(
/* Begin PNphpBB2 Module (Online/Offline status mod) */
		'POSTER_STATUS' => $status['text'],
		'POSTER_STATUS_IMG' => $status['image'],
/* End PNphpBB2 Module (Online/Offline status mod) */
		'ROW_COLOR' => '#' . $row_color,
		'ROW_CLASS' => $row_class,
/* Begin PNphpBB2 Module */
/*		'POSTER_NAME' => $poster, */
		'POSTER_NAME' => $poster_profile,
/* End PNphpBB2 Module */
...
// Begin PNphpBB2 Module
	...
	{
		$template->assign_block_vars('postrow.posticon', array(
			'ICON' => "<img " . $board_config['posticons_size'] . " src=\"" . $board_config['path_posticons'] . "/image_" . $postrow[$i]['post_icon'] . ".gif\" alt=\"" . $postrow[$i]['post_icon'] . "\" border=\"0\">")
		);
	}
// End PNphpBB2 Module
...
// Begin PNphpBB2 Module
if ($board_config['pnphpbb2_quickreply'] > 0)
{
	if ( $is_auth['auth_reply'] && !( $forum_topic_data['forum_status'] == FORUM_LOCKED ) && !( $forum_topic_data['topic_status'] == TOPIC_LOCKED ) || $userdata['user_level'] == ADMIN || $is_auth['auth_mod'] )
	{

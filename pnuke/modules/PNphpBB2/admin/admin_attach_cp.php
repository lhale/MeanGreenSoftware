...
//	$module['Attachments']['Control_Panel'] = $filename;
	$phpbb_module['Attachments']['Control_Panel'] = $filename;
// End PNphpBB2 Module
	return;
}
...
/*			'L_STATISTICS_FOR_USER'	=> sprintf($lang['Statistics_for_user'], $username)) */
			'L_STATISTICS_FOR_USER' => sprintf($lang['Statistics_for_user'], pnVarPrepForDisplay($username)))
/* End PNphpBB2 Module */
		);
...
*/
// -- Add --
	generate_pagination('admin_attach_cp.' . $phpEx . '?view=' . $view . '&mode=' . $mode . '&order=' . $sort_order . '&uid=' . $uid, $total_rows, $board_config['topics_per_page'], $start);
// End PNphpBB2 Module
}

...
$search_modules[] = array(
    'title' => 'pnCPG',
    'func_search' => 'search2_pncpg',
    'func_opt' => 'search2_pncpg_opt'
);

function search2_pncpg_opt() {
   ...
   global
    ...
    $output->SetInputMode(_PNH_VERBATIMINPUT);
     ...
     if (pnSecAuthAction(0, 'pnCPG::', '::', ACCESS_READ)) {
		...
		<input type=\"checkbox\" name=\"active_cpg2\" id=\"active_cpg2\" value=\"1\" checked=\"checked\" tabindex=\"0\" />
		&nbsp;<label for=\"active_cpg2\">Search Coppermine Albums</label>
		</span></td></tr></table>");
    }
...
function search2_pncpg() {

    list($q,
         $bool,
         $startnum,
         $total,
         $active_cpg2) = pnVarCleanFromInput('q',
                                             'bool',
                                             'startnum',
                                             'total',
                                             'active_cpg2');

    if (empty($active_cpg2)) {
        return;
    }
...
$query = "SELECT  $cpgprf"._albums.".title,$cpgprf"._albums.".aid ,$cpgprf"._albums.".description  from $cpgprf"._albums."   WHERE  (find_in_set( visibility, '$view_list') > 0) and ";
    ...
    foreach($w as $word) {
        ...
        $query .= '(';
        // faqs
        $query .= "$cpgprf"._albums.".title LIKE '".pnVarPrepForStore($word)."' OR \n";
        $query .= "$cpgprf"._albums.".description LIKE '".pnVarPrepForStore($word)."'\n";
        $query .= ')';
    ...
    if ($total<>0) {
        ...
        $output->Text( 'Coppermine Albums: ' . $total . ' ' . _SEARCHRESULTS);
        $output->SetInputMode(_PNH_VERBATIMINPUT);
        // Rebuild the search string from previous information
		$url="index.php?module=pnCPG&func=view&soort=1&album=$aid";
       $output->Text("<dl>");
        //while(!$result->EOF) {
		$hits= 0;
while (list($name, $title,$aid, $description,  $pid) = mysql_fetch_row($result) and $hits<11 ) { 
	            $output->Text("<dt><a href=\"index.php?module=pnCPG&func=view&soort=1&album=$aid\">".pnVarPrepForDisplay($name)."</a></dt>");
        ...
        $output->SetInputMode(_PNH_VERBATIMINPUT);
        $output->Text('No Results within Coppermine Albums');

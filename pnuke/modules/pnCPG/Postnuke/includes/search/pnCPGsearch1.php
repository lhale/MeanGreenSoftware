...
$search_modules[] = array(
    'title' => 'pnCPG',
    'func_search' => 'search1_pncpg',
    'func_opt' => 'search1_pncpg_opt'
);

function search1_pncpg_opt() {
   ...
   global
    ...
    $output->SetInputMode(_PNH_VERBATIMINPUT);
     ...
     if (pnSecAuthAction(0, 'pnCPG::', '::', ACCESS_READ)) {
		...
		<input type=\"checkbox\" name=\"active_cpg\" id=\"active_cpg\" value=\"1\" checked=\"checked\" tabindex=\"0\" />
		&nbsp;<label for=\"active_cpg\">Search Coppermine Pictures</label>
		</span></td></tr></table>");
    }
...
function search1_pncpg() {

    list($q,
         $bool,
         $startnum,
         $total,
         $active_cpg) = pnVarCleanFromInput('q',
                                             'bool',
                                             'startnum',
                                             'total',
                                             'active_cpg');
...
$query = "SELECT $cpgprf"._pictures.".filename, $cpgprf"._pictures.".title,$cpgprf"._albums.".aid ,$cpgprf"._albums.".description,$cpgprf"._pictures.".pid  from $cpgprf"._pictures.",$cpgprf"._albums."   WHERE $cpgprf"._pictures.".aid = $cpgprf"._albums.".aid and $cpgprf"._pictures.".approved = 'YES' and (find_in_set( visibility, '$view_list') > 0) and ";
    ...
    foreach($w as $word) {
        ...
        $query .= '(';
        // faqs
        $query .= "$cpgprf"._pictures.".title LIKE '".pnVarPrepForStore($word)."' OR \n";
        $query .= "$cpgprf"._pictures.".filename LIKE '".pnVarPrepForStore($word)."'\n";
        $query .= ')';
    ...
    if ($total<>0) {
        ...
        $output->Text( 'Coppermine Pictures: ' . $total . ' ' . _SEARCHRESULTS);
        $output->SetInputMode(_PNH_VERBATIMINPUT);
...
while (list($name, $title,$aid, $description,  $pid) = mysql_fetch_row($result) and $hits<11 ) { 
        ...
        $output->SetInputMode(_PNH_VERBATIMINPUT);
        $output->Text('No Results within Coppermine Pictures');

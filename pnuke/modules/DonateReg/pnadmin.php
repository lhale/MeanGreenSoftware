...
/**
 * view entry items
 * Display basic info about all entries in a paginated style
 */
...
// LDH 			_POSTCALENDAR
        ...
        2);    // LDH - the 2 param is the border size (default = 0 )
//    echo "sizeof(items)=" . count($items). "<br />";
        ...
        if (pnSecAuthAction(0, 'DonateReg::', ":$item[categoryid]:$item[donate_selectionid]", ACCESS_READ)) 
        {
			...
			*/
            ...
            $totalregs = pnModAPIFunc('DonateReg', 'user', 'countregistrationsforevent',
                array('donate_selectionid' => $item['donate_selectionid']));
            // LDH - URL convert BUG: if last param == 0, then you'll get the full URL string instead
            if ($totalregs == 0 )
                $totalregs = "0 ";
...
/* LDH - later 
    	if(pnModAvailable( 'PostCalender'))    // LDH - disabled for now via misspelling
    	{
            $output->SetInputMode(_PNH_VERBATIMINPUT);
	...
	LDH - Next three lines elemental in spitting out the whole row (???)*/
        {  				
                $output->SetOutputMode(_PNH_KEEPOUTPUT);
                $output->TableAddRow($row);
                $output->SetInputMode(_PNH_PARSEINPUT);
        }
      }    // end pnSecAuthAction section
      else
...
//echo $total;
    ...
    // Return the output that has been generated by this function
    return $output->GetOutput();
} // donatereg_admin_viewevents()


/**
 * display entry item
 * Display detailed info about a specific entry
 */
...
*/
	...
	/* LDH - maybe expand location later. For now, it's just a location name
	// $loc = serialize(compact('event_location', 'event_street1', 'event_street2',
	'event_city', 'event_state', 'event_postal'));
*/
    $loc = $event_location;    // LDH - for now, use this to describe the donate location
...
*/
    ...
    if ($donate_selectionid != false) 
    {
        ...
        $output->SetOutputMode(_PNH_KEEPOUTPUT);
        $output->SetInputMode(_PNH_VERBATIMINPUT);
        // Display the new entry item using a user level function
		$output->Text(pnModFunc('DonateReg',
        					'user',
							'displayevent',
...
{ 
    ...
    $output->TableAddrow($row, 'left');
    $output->SetInputMode(_PNH_PARSEINPUT);
    $output->FormStart(pnModURL('DonateReg', 'admin', 'createevent'));     // LDH - Whathell is this extra doing here ???
    // Category selection
    // Load API.  All of the actual work for the creation of the new item is
    // done within the API, so we need to load that in before we can do
    // anything.  If the API fails to load an appropriate error message is
...
/* LDH - No need for registration style times    
    $row = array();
    $output->SetOutputMode(_PNH_RETURNOUTPUT);
    $row[] = $output->Text(pnVarPrepForDisplay(_REGSTARTDATETM));
...
*/
    ...
    $output->TableAddrow($row, 'left');
    $output->SetInputMode(_PNH_PARSEINPUT);
/* LDH - We don't charge donors fees
    // Fee
    $row = array();
    $output->SetOutputMode(_PNH_RETURNOUTPUT);
    $row[] = $output->Text(pnVarPrepForDisplay(_EVENTFEE . " (" . _EVENTREGCURRENCY . ")"));
...
*/
    ...
    if ($updated == false) {
        ...
        $output->Title(_EVENTUPDATEFAILED);
        return $output->GetOutput();
    } 

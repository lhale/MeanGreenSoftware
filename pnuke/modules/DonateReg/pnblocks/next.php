...
function DonateReg_nextblock_init()
{
    pnSecAddSchema('donatereg:nextblock:', 'Block title::');
}


function DonateReg_nextblock_info()
{
...
function DonateReg_nextblock_display($blockinfo)
{
...
if ($items == false) {
    ...
    foreach ($items as $item) {
		...
		if (pnSecAuthAction(0,
                            'DonateReg::events',
                            " $item[addedby]:$item[category]:$item[donate_selectionid] ",
                            ACCESS_READ)) {
...
function DonateReg_nextblock_modify($blockinfo)
{
    $output = new pnHTML();
    $vars = pnBlockVarsFromContent($blockinfo['content']);
...
function DonateReg_nextblock_update($blockinfo)
{

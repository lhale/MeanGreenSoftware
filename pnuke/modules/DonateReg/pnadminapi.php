...
newevent($name, $orgname, $category, $startdate, $enddate
		 ...
		 $description, $max_donors, $fee, $phone, $eventlogo, $eventicon
		 $commentfieldlabel, $dateadded, $addeby)
	returns donate_selectionid or false
deleteevent($donate_selectionid)
	returns true or false
updateevent($donate_selectionid, $name, $orgname, $category, $startdate, $enddate
	        $reg_startdate, $reg_enddate, $location, $summary, $header
		    $description, $max_donors, $fee, $phone, $eventlogo, $eventicon
...
function donatereg_adminapi_createcategory($args)
{
    extract($args);
    ...
    // item
    pnModCallHooks('admin_item', 'create', $catid, "createcategory"); 
    // Return the id of the newly created item to the calling process
    return $categoryid;
} 

function donatereg_adminapi_deletecategory($args)
{ 
    ...
    if (!pnSecAuthAction(0, 'DonateReg::category', "$category[name]::$categoryid", ACCESS_DELETE)) {
    ...
    $eventcolumn = $pntable['donate_selections_column'];    // LDH - chg to selection_column
    ...
    // LDH - changed, not any more. There isn't any delete hook function anyways
    pnModCallHooks('admin_item', 'delete', $categoryid, "deletecategory"); 
    // Let the calling process know that we have finished successfully
    return true;
} 

function donatereg_adminapi_updatecategory($args)
{ 
    // Get arguments from argument array - all arguments to this function
    // should be obtained from the $args array, getting them from other
    // places such as the environment is not allowed, as that makes
    ...
    // appropriate item.  If the item does not exist we post an appropriate
    // message and return
    // $category = pnModAPIFunc('DonateReg', 'user','getcategory', array('categoryid' => $categoryid));
    ...
    if (!pnSecAuthAction(0, 'DonateReg::category', "$item[name]::$categoryid", ACCESS_EDIT)) {
        pnSessionSetVar('errormsg', _TEMPLATENOAUTH);
        return false;
    } 
    if (!pnSecAuthAction(0, 'DonateReg::category', "$name::$categoryid", ACCESS_EDIT)) {
        pnSessionSetVar('errormsg', _TEMPLATENOAUTH);
        return false;
    } 
    // Get datbase setup - note that both pnDBGetConn() and pnDBGetTables()
...
function donatereg_adminapi_getallcategories()
{
    $categories = array();
...
function donatereg_adminapi_getallregistrations()
{
    $categories = array();
...
function donatereg_adminapi_createevent($args)
{ 
    // Get arguments from argument array - all arguments to this function
    // should be obtained from the $args array, getting them from other
    // places such as the environment is not allowed, as that makes
...
*/	
    ...
    $sql = "INSERT INTO $eventtable (
            ...
            VALUES (
	          ...
	          NOW(),
			  '" . pnvarPrepForStore($addedby) . "',
	          NOW(),
			  '" . pnvarPrepForStore($changedby) . "',
    ...
    // very unlikely, that this is different from $nextId as obtained
    // above, but it is better to be safe than sorry in this situation
    $donate_selectionid = $dbconn->PO_Insert_ID($eventtable, $eventcolumn['donate_selectionid']); 
    // Let any hooks know that we have created a new item.  As this is a
    // create hook we're passing 'tid' as the extra info, which is the
    // argument that all of the other functions use to reference this
    // item
    pnModCallHooks('admin_item', 'create', $donate_selectionid, "createevent"); 
    // Return the id of the newly created item to the calling process
    return $donate_selectionid;
} // donatereg_adminapi_createevent
 ...
 */
function donatereg_adminapi_deleteevent($args)
{ 
    // Get arguments from argument array - all arguments to this function
    // should be obtained from the $args array, getting them from other
    // places such as the environment is not allowed, as that makes
    ...
    // name to complete the instance information so this is the first
    // chance we get to do the check
    if (!pnSecAuthAction(0, 'DonateReg::event', "$event[name]:$event[category]:$donate_selectionid", ACCESS_DELETE)) {
        pnSessionSetVar('errormsg', _EVENTREGNOAUTH);
        return false;
    } 
    // Get datbase setup - note that both pnDBGetConn() and pnDBGetTables()
    ...
    // debug operation if it is ever needed
    $sql = "DELETE FROM $registrationstable
            WHERE $registrationscolumn[donate_selectionid] = '" . pnVarPrepForStore($donate_selectionid) . "'";
    $dbconn->Execute($sql);
    ...
    // delete hook we're not passing any extra info - 
    // LDH - changed, not any more. There isn't any delete hook function anyways
    pnModCallHooks('admin_item', 'delete', $donate_selectionid, "deleteselection"); 
    // Let the calling process know that we have finished successfully
    return true;
} 
 ...
 */
function donatereg_adminapi_updateevent($args)
{ 
    // Get arguments from argument array - all arguments to this function
    // should be obtained from the $args array, getting them from other
    // places such as the environment is not allowed, as that makes
...
//	echo "$donate_selectionid,$eventname,$orgname,$category,$location,$country,$summary,$header,$description,$maxregistrations,$output,$phone,$notifyemail,$commentfieldlabel,$changedby,";
    // Argument check - make sure that all required arguments are present,
    // if not then set an appropriate error message and return
    if ( (!isset($donate_selectionid)) || (!isset($eventname)) || (!isset($orgname)) || 
	     (!isset($category)) || (!isset($location)) || (!isset($country)) ||
		 (!isset($summary)) || (!isset($header)) || (!isset($description)) || 
		 (!isset($maxregistrations)) || (!isset($output)) ||
		 (!isset($phone)) || (!isset($notifyemail)) ||
		 (!isset($commentfieldlabel)) || (!isset($changedby))) {
    ...
    // suitable permissions to edit the item otherwise people can potentially
    // edit areas to which they do not have suitable access
    if (!pnSecAuthAction(0, 'DonateReg::event', ":$category:$donate_selectionid", ACCESS_EDIT)) {
        pnSessionSetVar('errormsg', _EVENTREGNOAUTH);
        return false;
    } 
    // Get datbase setup - note that both pnDBGetConn() and pnDBGetTables()
    ...
    $sql = "UPDATE $eventtable 
            SET $eventcolumn[name] = '" . pnvarPrepForStore($eventname) . "',
               ...
               $eventcolumn[location] = '" . pnvarPrepForStore($location) . "',
                ...
                $eventcolumn[max_donors] = '" . pnvarPrepForStore($maxregistrations) . "',
                $eventcolumn[changedby] = " . pnvarPrepForStore($changedby) . ",
                $eventcolumn[datechanged] = " . "NOW()" . ",
 ...
 * @param  $args ['number'] the new number of the item
 */
function donatereg_adminapi_updateevent_orig($args)
{ 
    // Get arguments from argument array - all arguments to this function
    // should be obtained from the $args array, getting them from other
    // places such as the environment is not allowed, as that makes
    ...
    // Argument check - make sure that all required arguments are present,
    // if not then set an appropriate error message and return
    /*    if ( (!isset($donate_selectionid)) || (!isset($name)) || (!isset($orgname)) || 
	     (!isset($category)) || (!isset($startdate)) || (!isset($enddate)) || 
		 (!isset($reg_startdate)) || (!isset($reg_enddate)) || (!isset($location)) ||
		 (!isset($summary)) || (!isset($header)) || (!isset($description)) || 
		 (!isset($registrations)) || (!isset($max_donors)) || (!isset($fee)) ||
...
function donatereg_adminapi_getallevents($args)
{ 
    ...
    // LDH - Later, sort entries out into categories
...
function donatereg_adminapi_preppostcalendarevent($args) {
extract($args);
...
// Load API.  All of the actual work for obtaining information on the items
	...
	*/
	...
	$duration = pnModAPIFunc('DonateReg',
        'admin',
    ...
    //array('event_start' =>$event_startstamp, 'event_end' => $event_endstamp) already have the timestamp now
		...
		$url= pnModURL('DonateReg','user','displayevent',array('donate_selectionid' => $donate_selectionid));
		//echo "output=[".$url."]<br>";
		// LDH - added a little hyperlink prominence so that folks could see it (I missed it several times)
		$url = "<br />" . '<a href="' . $url . '" style="font-size: x-large; font-weight: bold;">' . _REGISTER . '</a>';
		// echo "output=[".$url."]<br>";
		...
		$pc_event = array('event_subject' => $event[name],
							...
							'event_category' => pnModAPIFunc('DonateReg',
								'admin',
...
function donatereg_adminapi_guesspostcalendarduration($args) {
extract($args);
/* echo "Start time: ";
...
//Function to find what PostCalendar category matches an DonateReg Category
// LDH - unsupported (no DonateReg dates supported)
function donatereg_adminapi_findpostcalendarcategory($eventreg_catid)
{
...
list($dbconn) = pnDBGetConn();
    ...
    $sql = "Select $donate_categories_column[pc_categoryid] 
    FROM $donate_categories_table
    WHERE $donate_categories_column[categoryid] = $eventreg_catid";
    //echo $sql."<br>";
...
DonateReg categories to PostCalendar categories and better integrate the two modules (i.e when you use DonateReg to create a PostCalendar item it has the correct category */
function donatereg_adminapi_getpostcalendarcategories()
{
    $categories = array();
 ...
 *  2, admin_item, create, DonateReg, NULL , NO_GUI, donatereg, admin , createconfirm
 */
function donatereg_adminapi_createconfirm ($args)
{
//    array('objectid' => $hookid,
//		'extrainfo' => $extrainfo);
    extract ($args);    // nope - no worky
        list($id,
        $extrainfo) = 
        pnVarCleanFromInput('objectid',
        'extrainfo');   // nope - no worky
 //   echo "HOOK Called: id=" . $id . ",extra=" .  $extrainfo . "<br />"   ;
        // Somebody needs to tell me why the hell the above doesn't work. Do typical brute force way:
    if ( $id == null || $id == "")
    {
        $id = $args['objectid'];
        $extrainfo = $args['extrainfo'];    // extrainfo doesn't appear to be passed in (bogus)
        $hookid = $args['objectid'];
    }
    ...
    // There is really nothing to confirm, this is just current exercise for future use
//    echo "objectid=>$hookid,extrainfo=>$extrainfo,id=>$id";
//    (Should get something like "objectid=>15,extrainfo=>createevent,id=>15" )

    switch($extrainfo) {
		case "createevent":
		{
//		    echo "Creation of event selection#" . $id . " confirmed";
		    break;
		}
		case "createcategory":
		{
//		    echo "Creation of category selection#" . $id . " confirmed";
		    break;
		}
    }
}  // donatereg_adminapi_createconfirm()

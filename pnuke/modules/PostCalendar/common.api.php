<?php
/**
 *  $Id: common.api.php,v 1.7 2006/01/31 10:40:36 larsneo Exp $
 *
...
function pcGetTopicName($topicid)
{
    ...
    $sql = "SELECT $topics_column[topicname]
            FROM $topics_table
			WHERE $topics_column[topicid] = '".(int)pnVarPrepForStore($topicid)."'";
...
<script type="text/javascript" language="JavaScript" src="modules/$pcDir/pnincludes/overlib_mini.js">
 ...
 *	@param $args array of event data
 *	@return bool true on success : false on failure;
 * LDH: This is called indirectly from buildSubmitForm thru the 'submit-admin' PC url mechanics (UGHH)
 */
function postcalendar_userapi_submitEvent($args) 
{
    ...
    if( (bool) _SETTING_DIRECT_SUBMIT || 
		(bool) PC_ACCESS_ADMIN || 
		($event_sharing != SHARING_GLOBAL) ) {
	...
	/* causes problem when admin edits user's calendar
	if(pnUserLoggedIn()) {
	...
	*/
	...
	/*
	DW (me <-at-> denniswarnr dot net) 03-26-2004
	Changed code so that admins can edit users' appointments
	*/
	if(pnUserLoggedIn()) {
		// Checks to see if this is a new event or not
		if ($pc_event_id != ""){
			// Since this is not a new event, we go
			// to database to see who owns it
			$sql = "SELECT pc_aid
					FROM $pntable[postcalendar_events]
					WHERE pc_eid = '".(int)pnVarPrepForStore($pc_event_id)."'";

			$result = $dbconn->Execute($sql);
			$event_userid_from_db = $result->fields[0];

			$current_user_id = pnUserGetVar('uid');

			// If the current user editing the event
			// is the one that owns it
			if ($event_userid_from_db == $current_user_id){
				$event_userid = $current_user_id;
				// Else change the id to the user that owns the event
				// which we get from the database
			} else {
				$event_userid = $event_userid_from_db;
			}
		} else {
			$event_userid = pnUserGetVar('uid');
		}
	...
	list($event_subject,$event_desc,$event_topic,$startDate,$endDate,
		 ...
		 $event_recurrspec,$event_duration,$event_sharing,$event_userid,
...
/* LDH - needed for debugging the stoopid primary index and pc_topic SQL error
    print("CMD=\"" . "INSERT INTO $pntable[postcalendar_events] (
                	pc_eid,
					pc_title,
                	pc_time,
                	pc_hometext,
 ...
 */   
    	...
    	/*
    	 * LDH - Since pc_topic is an INT(3), and since '' is sometimes passed to it, 
    	 * You MUST always make sure that topics are enabled to get past this SQL error
    	 */
...
function postcalendar_userapi_buildSubmitForm($args,$admin=false)
{
	...
	$form_submit = '<select name="form_action">
                    ...
                    <option value="commit">'._PC_EVENT_SUBMIT.'</option>
                    </select>'.$authkey.'<input type="submit" name="submit" value="go">' ;
...
function &postcalendar_userapi_pcGetEventDetails($eid) 
{
    ...
    $sql =  "SELECT DISTINCT e.pc_eid,
			 ...
			 WHERE  e.pc_eid = '".pnVarPrepForStore($eid)."' AND c.pc_catid = e.pc_catid";
    ...
    list($event['eid'],        $event['uname'],       $event['catid'],
         ...
         $event['catcolor'],   $event['catname'],     $event['catdesc']) = $result->fields;
	...
	//''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
	$result->Close();
	// LDH - debug. Must do a value copy because of the pass by ref stuff
//	    $hometext = $event[hometext];
//       echo "hometext=$hometext";
...
// echo "hometext=$event[hometext]";
		...
		if(pnUserLoggedIn()) {
			$logged_in_uname = pnUserGetVar('uname');
		} else {
			$logged_in_uname = '';
		}
		$can_edit = false;
		if (pnSecAuthAction(0, 'PostCalendar::', '::', ACCESS_ADD) && $logged_in_uname == $event['uname']) {
...
{   
    // lets get the module's information
    $modinfo = pnModGetInfo(pnModGetIDFromName(__POSTCALENDAR__));
    $footer = "<p align=\"right\"><a href=\"http://noc.postnuke.com/projects/postcalendar\">PostCalendar v$modinfo[version]</a></p>";
    return $footer;
}
...
function sort_byCategoryA($a,$b) {
	...
	// rjf 7/20/2004 Begin: if the categories are equal, sort based on start time
	elseif($a['catname'] == $b['catname']) return sort_byTimeA($a,$b);
	// rjf 7/20/2004 End:
}
function sort_byCategoryD($a,$b) {
	...
	elseif($a['catname'] > $b['catname']) return -1;
	// rjf 7/20/2004 Begin: if the categories are equal, sort based on start time
	elseif($a['catname'] == $b['catname']) return sort_byTimeD($a,$b);
	// rjf 7/20/2004 End:
}

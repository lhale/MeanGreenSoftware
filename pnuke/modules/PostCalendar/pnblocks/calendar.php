...
/**
 *  $Id: calendar.php,v 1.5 2006/01/31 10:40:36 larsneo Exp $
 *
...
function postcalendar_calendarblock_display($blockinfo)
{
	...
	if(!$templates_cached) {
    	...
    	$pc_prev = pnVarPrepForDisplay(pnModURL(__POSTCALENDAR__,'user','view',array('tplview'=>$template_view,'viewtype'=>'month','Date'=>$prev_month)));
    	$pc_next = pnVarPrepForDisplay(pnModURL(__POSTCALENDAR__,'user','view',array('tplview'=>$template_view,'viewtype'=>'month','Date'=>$next_month)));
    	$pc_month_name = pnModAPIFunc(__POSTCALENDAR__, 'user', 'getmonthname', array('Date'=>mktime(0,0,0,$the_month,$the_day,$the_year)));
    	$month_link_url = pnVarPrepForDisplay(pnModURL(__POSTCALENDAR__, 'user', 'view', array('tplview'=>$template_view,'viewtype'=>'month','Date'=>date('Ymd',mktime(0,0,0,$the_month,1,$the_year)))));

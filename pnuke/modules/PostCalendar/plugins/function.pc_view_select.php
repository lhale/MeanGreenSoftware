<?php
/**
 *  $Id: function.pc_view_select.php,v 1.2 2004/02/11 19:16:00 larsneo Exp $
 *
...
function smarty_function_pc_view_select($args) 
{
    ...
    $options = "<select id=\"tplview\" name=\"tplview\" class=\"$args[class]\">";
    $selected = $tplview;
    for($t=0;$t<$tcount;$t++) {
    ...
    $submit = '<input type="submit" name="submit" value="'.$args['label'].'" class="'.$args['class'].'" />';
    // build the form
    if($t > 1) {
        echo $options,$submit;
    } 

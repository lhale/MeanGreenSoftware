<?php
/**
 *  $Id: function.pc_filter.php,v 1.2 2004/02/11 19:12:44 larsneo Exp $
 *
...
function smarty_function_pc_filter($args, &$smarty)
{
	...
	if(in_array('user',$types)) {
		...
		$sql = "SELECT DISTINCT pc_informant
	 			FROM $pntable[postcalendar_events]
				ORDER BY pc_informant";
		...
		if($result !== false) { 
			...
			for(;!$result->EOF;$result->MoveNext()) {
        		...
        		$useroptions .= "<option value=\"".$result->fields[0]."\" $sel class=\"$class\">".$result->fields[0]."</option>";
			}
    		$useroptions .= '</select>';
			$result->Close();
		}
	...
	if(in_array('category',$types)) {
		...
		foreach($categories as $c) {
        	...
        	$catoptions .= "<option value=\"$c[id]\" $sel class=\"$class\">$c[name]</option>";
		}
		$catoptions .= '</select>';
	}
	//================================================================

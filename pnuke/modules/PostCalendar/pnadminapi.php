...
/**
 *  $Id: pnadminapi.php,v 1.1 2004/02/11 17:25:28 larsneo Exp $
 *
...
function postcalendar_adminapi_getAdminListEvents($args) 
{
	...
	$sql = "SELECT pc_eid,
                   ...
                   pc_time 
            FROM   $events_table
            WHERE  pc_eventstatus = '".pnVarPrepForStore($type)."' ";
    if($sort == 'time') {
...
function postcalendar_adminapi_addCategories($args) 
{
    ...
    print("<br>Name=" . $name . ",Desc=" . $desc . ",Color=" . $color . ",<br>");
    // LDH - Whether an InnoDB or MyISAM tbl, you cannot have '' for catid
    print("CMD=\"" . "INSERT INTO $pntable[postcalendar_categories] 
                                (pc_catid,pc_catname,pc_catdesc,pc_catcolor)
                                VALUES (NULL,'$name','$desc','$color')\"");
    $name = pnVarPrepForStore($name);
    $desc = pnVarPrepForStore($desc);
    $color = pnVarPrepForStore($color);
    
    $result = $dbconn->Execute("INSERT INTO $pntable[postcalendar_categories] 
                                (pc_catid,pc_catname,pc_catdesc,pc_catcolor)
                                VALUES (NULL,'$name','$desc','$color')");
    if($result === false) {
        return false;

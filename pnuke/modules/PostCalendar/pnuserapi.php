...
/**
 *  $Id: pnuserapi.php,v 1.4 2006/01/31 10:40:36 larsneo Exp $
 *
...
{   
	...
	if(!$tpl->is_cached("$template_name/views/$viewtype/$template_view_load.html",$cacheid)) {
    	...
    	$pc_prev = pnVarPrepForDisplay(pnModURL(__POSTCALENDAR__,'user','view',
    	...
    	$pc_next = pnVarPrepForDisplay(pnModURL(__POSTCALENDAR__,'user','view',
                        	array('tplview'=>$template_view,
                            	  'viewtype'=>'month',
                            	  'Date'=>$next_month,
                            	  'pc_username'=>$pc_username,
    	...
    	$pc_prev_day = pnVarPrepForDisplay(pnModURL(__POSTCALENDAR__,'user','view',
                            	 array('tplview'=>$template_view,
                                	   'viewtype'=>'day',
                                	   'Date'=>$prev_day,
                                	   'pc_username'=>$pc_username,
    	...
    	$pc_next_day = pnVarPrepForDisplay(pnModURL(__POSTCALENDAR__,'user','view',
                            	array('tplview'=>$template_view,
                                	  'viewtype'=>'day',
                                	  'Date'=>$next_day,
                                	  'pc_username'=>$pc_username,
    	...
    	$pc_prev_week = pnVarPrepForDisplay(pnModURL(__POSTCALENDAR__,'user','view',
                            	 array('viewtype'=>'week',
                                	   'Date'=>$prev_week,
                                	   'pc_username'=>$pc_username,
								  	   'pc_category'=>$category,
								  	   'pc_topic'=>$topic)));
    	$pc_next_week = pnVarPrepForDisplay(pnModURL(__POSTCALENDAR__,'user','view', 
                            	 array('viewtype'=>'week',
                                	   'Date'=>$next_week,
                                	   'pc_username'=>$pc_username,
								  	   'pc_category'=>$category,
    	...
    	$pc_prev_year = pnVarPrepForDisplay(pnModURL(__POSTCALENDAR__,'user','view',
                            	 array('viewtype'=>'year',
                                	   'Date'=>$prev_year,
                                	   'pc_username'=>$pc_username,
								  	   'pc_category'=>$category,
								  	   'pc_topic'=>$topic)));
    	$pc_next_year = pnVarPrepForDisplay(pnModURL(__POSTCALENDAR__,'user','view',
                            	 array('viewtype'=>'year',
                                	   'Date'=>$next_year,
                                	   'pc_username'=>$pc_username,
								  	   'pc_category'=>$category,
...
function postcalendar_userapi_eventPreview($args)
{
    ...
    if(_SETTING_DISPLAY_TOPICS) {
    	...
    	$topicsql = "SELECT $topics_column[topictext],$topics_column[topicimage]
            	 	 FROM $topics_table 
            	 	 WHERE $topics_column[topicid] = '".(int)pnVarPrepForStore($event[topic])."'
            	 	 LIMIT 1";

<?php
// $Id: banners.php,v 1.3 2005/11/07 21:50:04 jazzie Exp $
// ----------------------------------------------------------------------
...
// variable that need to be defined for the system
	$modversion['file_name'] = 'banners'; // the name of this php file
	$modversion['mod_name'] = 'Banners'; // the name of the module you are writing this for
	...
	$modversion['function_name'] = 'random_banner'; // the name of the function listed below
	$modversion['description'] = 'Random Banners'; // brief description of this module
...
function random_banner_html($mod_id, $nl_url, $ModName) {
...
// name the lang file the same as this file
	  include("modules/$ModName/modules/lang/$language/banners.php");
...
// query the database and generate your output in the amount of mod_qty
	$sql_test = mysql_query("SELECT pn_type FROM $pntable[banner] WHERE pn_type = '0' limit 1");
	if($sql_test) {
		$sql = mysql_query("SELECT pn_imageurl, pn_clickurl FROM $pntable[banner] 
			WHERE pn_type = '0' ORDER BY RAND() limit 1");
		} else {
		$sql = mysql_query("SELECT pn_imageurl, pn_clickurl FROM $pntable[banner] 
			ORDER BY RAND() limit 1");
		}
	list($pn_imageurl, $pn_clickurl) = mysql_fetch_row($sql);
	if($mod_data[1] == 1) {
		$output .= "<div align=\"center\"><a href=\"$pn_clickurl\" target=\"_blank\"><img src=\"$pn_imageurl\" border=\"0\"></a></div><br>\n";
		} else {
		$output .= "<div align=\"center\"><a href=\"$pn_clickurl\" target=\"_blank\"><img src=\"$nl_url/$pn_imageurl\" border=\"0\"></a></div><br>\n";
		}
	$output .= "<br>\n";
...
function random_banner_text($mod_id, $nl_url, $ModName) {
...
function random_banner_admin($mod_id, $ModName) {
...
// the admin part
	...
	echo "></td>\n"
		."<td class=\"pn-normal\"><b>"._ARENOTFULLURL."</b></td>\n"
		."</tr>\n"
		."<tr>\n"
	...
	echo ">"
		."</td>\n"
		."<td class=\"pn-normal\"><b>"._AREFULLURL."</b></td>\n"
		."</tr>\n"
		."</table>\n"
		."\n";

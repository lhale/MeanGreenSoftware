<?php
// $Id: enhdownloads.php,v 1.2 2005/11/07 21:57:30 jazzie Exp $
// ----------------------------------------------------------------------
...
// variable that need to be defined for the system
	$modversion['file_name'] = 'enhdownloads'; // the name of this php file
	...
	$modversion['function_name'] = 'enh_downloads'; // the name of the function listed below
	$modversion['description'] = 'Enhanced Downloads'; // brief description of this module
	$modversion['author'] = 'foyleman, enhancment by Ozancin';
	$modversion['contact'] = 'http://canvas.anubix.net/';

// Initialize variables

function enh_downloads_init($ModName) {

	$language = pnConfigGetVar('language');

	// name the lang file the same as this file
	include("modules/$ModName/modules/lang/$language/enhdownloads.php");

	// install common look and feel
	include("modules/$ModName/modules/common/enh.php");

	$data = ""._LATESTDOWNLOADS."|"._BORDERCOLOR_VALUE."|"._BGCOLOR1_VALUE."|"._BGCOLOR2_VALUE."|"._BORDERWIDTH_VALUE."|modules/images/enhdownloads/downloads.gif|"._BULLETIMG_VALUE."";

	return $data;
}

	$modversion['mod_data'] .= enh_downloads_init($ModName);

// function named as above, in this case the format is for the HTML part of the mailer

function enh_downloads_html($mod_id, $nl_url, $ModName) {
...
// create the array from the stored variables
	...
	while(list($pn_lid, $pn_title, $pn_description, $pn_filesize) = mysql_fetch_row($sql)) {
		...
		$output .= "<td><a href=\"$nl_url/modules.php?op=modload&name=Downloads&file=index&req=viewdownloaddetails&lid=$pn_lid&ttitle=$pn_title\" target=\"_blank\"><b><font style=\"font-size: 13px\" size=\"4\">$pn_title</font></b></a></td>\n";

		$output .= "</tr></table></div>\n";

		$output .= "<div align=\"left\"> <table border=\"0\" width=\"100%\" cellspacing=\"0\" cellpadding=\"3\"><tr>\n";

		$output .= "<td><font style=\"FONT-SIZE: 11px\">$pn_description</font>\n";
		if (!empty($pn_notes)) {
		...
		$output .= "<td valign=\"middle\"><b><font style=\"FONT-SIZE: 11px\">"._FILESIZE.": $pn_filesize "._BYTES."</font></b></td>\n";
...
function enh_downloads_text($mod_id, $nl_url, $ModName) {
	global $prefix, $pntable;
	$dbconn =& pnDBGetConn(true);
	        $pntable =& pnDBGetTables();
...
function enh_downloads_admin($mod_id, $ModName) {
	global $prefix, $pntable;
	$dbconn =& pnDBGetConn(true);
	        $pntable =& pnDBGetTables();

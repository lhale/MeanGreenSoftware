<?php
// $Id: headlinessince.php,v 1.2 2005/11/07 21:57:30 jazzie Exp $
// ----------------------------------------------------------------------
...
// variable that need to be defined for the system
	$modversion['file_name'] = 'headlinessince'; // the name of this php file
	...
	$modversion['function_name'] = 'latest_headlinessince'; // the name of the function listed below
	$modversion['description'] = 'Latest News Headlines Since'; // brief description of this module
...
function latest_headlinessince_html($mod_id, $nl_url, $ModName) {
...
// name the lang file the same as this file
	include("modules/$ModName/modules/lang/$language/headlinessince.php");

// get the module setting from the database
		$modsql = mysql_query("SELECT arch_date FROM $prefix"._nl_archive." ORDER BY arch_date DESC limit 1");
		list($arch_date) = mysql_fetch_row($modsql);
		$arch_date = date("Y-m-d H:i:s",$arch_date);
		$sql = mysql_query("SELECT pn_sid, pn_title, pn_hometext, pn_bodytext, pn_informant, pn_notes, pn_time FROM $pntable[stories] 
			WHERE pn_ihome = '0' AND pn_time > '$arch_date' ORDER BY pn_time DESC");
...
// title of the page to show up
	$output ="<h3>"._LATESTHEADLINES.":</h3><blockquote>\n";
	...
	while(list($pn_sid, $pn_title, $pn_time, $pn_topic) = mysql_fetch_row($sql)) {
		...
		$output .= "<b><font style=\"FONT-SIZE: 11px\"><a href=\"$nl_url/modules.php?op=modload&name=News&file=article&sid=$pn_sid\" target=\"_blank\">$pn_title</a></forum></b><br>\n";
		}
...
function latest_headlinessince_text($mod_id, $nl_url, $ModName) {
	global $prefix, $pntable;
	$dbconn =& pnDBGetConn(true);
	        $pntable =& pnDBGetTables();

...
{
  if(!this._stylist) return alert("No Stylist");//false;
  alert(this.stylist.innerHTML);
...
function Stylist(editor, args)
{
  ...
  editor.registerPanel('stylist', 'right', this);
}
...
Stylist.prototype.drawPanelIn = function(element)
{
  var editor = this.editor;
  if(typeof editor.config.css_style != 'undefined' && && HTMLArea.objectProperties(editor.config.css_style).length)
  {
    HTMLArea.addClass(element, 'stylist');
    editor._stylist = element;
  }
}

Stylist.prototype.onUpdateToolbar = function()
{
  this.editor._fillStylist();
}

...
/**
 * Display Refund Return Policy
 * @author Larry Hale
 * @return string HTML output string
 */

function legal_user_refunds()
{
    // Security check - important to do this as early as possible to avoid
    // potential security holes or just too much wasted processing.  For the
    // main function we want to check that the user has at least edit privilege
    ...
    // check the option is active
	if (!pnModGetVar('legal', 'refundpolicy')) {
		return pnVarPrepForDisplay(_PPNOTACTIVE);
	}
	...
	// work out the template path
	if ($pnRender->template_exists($lang.'/legal_user_refunds.htm')) {
		$template = $lang.'/legal_user_refunds.htm';
	} else {
		$template = 'eng/legal_user_refunds.htm';
	}

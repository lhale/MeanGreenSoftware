<?php
// File: $Id: user.php 19963 2006-09-03 21:38:05Z larsneo $
// ----------------------------------------------------------------------
...
function newuser_user_register()
{
  ...
  if (pnConfigGetVar('reg_question') != '' && pnConfigGetVar('reg_answer') != '') {
    // if there is a question/answer configured display and test 	
    echo '<tr>'
    .'<td align="right"><strong><label for="reg_answer">' . pnVarPrepForDisplay(pnConfigGetVar('reg_question')) . '</label></strong></td>'."\n"
    .'<td><input type="text" name="reg_answer" id="reg_answer" size="'.pnVarPrepForDisplay($size).'" maxlength="60" tabindex="0" /></td>'."\n"
    .'</tr>';
  }
...
//    print("<br>reg_optitems=$reg_optitems<br>");    // LDH - looks like this is not set
    if ($reg_optitems) {    
        list($name,
             $url,
             $bio,
    ...
    if (!$reg_optitems) {
        ...
        $user_msnm = '';
        $dynadata = '';
        // LDH - ?WTF? this is missing the following query insert info:
        /*
         * LDH - dbg - user_viewemail is the real req var (checkbox nm)
         * 	pnUserGetVar doesn't get req var's, only ones fr pnUserSetVar()
        $user_viewemail = pnUserGetVar('user_viewemail');
        print("<br>user_viewemail=$user_viewemail");
        $user_viewemail_new_user = pnUserGetVar('user_viewemail_new_user');
        print("<br>user_viewemail_new_user=$user_viewemail_new_user");
         */
        
        /*
         * LDH - dbg - user_viewemail is the real req var (checkbox nm)
         * 	pnModGetVar doesn't get req var's, only ones fr pnModSetVar()
        $user_viewemail = pnModGetVar('NewUser', 'user_viewemail');
        print("<br>user_viewemail=$user_viewemail");
		*/
        // Try : pnSessionGetVar, (probably only w/  pnSessionSetVar()
        if ( ($user_viewemail = pnVarCleanFromInput('user_viewemail')) == '' )
            $user_viewemail = '0';
        /*
        print("<br>user_viewemail=$user_viewemail");
		*/
    ...
    if (pnConfigGetVar('reg_question') != '' && pnConfigGetVar('reg_answer') != '') {
        // if there is a question/answer configured display and test 	
        if (pnVarCleanFromInput('reg_answer') != pnConfigGetVar('reg_answer')) {
            $stop = _REGQUESTIONFAILED;
        }
    }
    ...
    // end of additional checks
        ...
        $result =& $dbconn->Execute("INSERT INTO $pntable[users] ($column[name], $column[uname], $column[email],
                           ...
                           '" . pnVarPrepForStore($user_viewemail) . "',$user_theme,

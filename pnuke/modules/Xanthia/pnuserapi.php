...
//        global $pnconfig;
//		$ModName = pnModGetName();   // dbg
    $menu_title = pnVarCleanFromInput('menu_title');
//		echo "$includi,allblks=" . isset($allblocks) . ",title=" . pnConfigGetVar('menu_title') . "/" . pnModGetVar($ModName, 'menu_title') . "/" . $menu_title;
//		echo "modNm=" . $ModName;
//    echo "<pre>\n Blocks_userapi_getall Call Btrc:\n";
...
//    echo "</pre>\n";
    $grn_menu_blk_chosen = false;
		...
		if ($includi == 'false') {
			...
			// LDH - Added left block menu capability - site context driven Main Menu :
			// If menu_title is specified on the URL, then the first main Core menu corresponding
			// to this title is displayed. A MeanGreen menu block is differentiated by using
			// the (unused - I think) url pn_blocks column, encoded with a 'MeanGreen' string
			foreach ($allblocks as $row) {		
                ...
                // LDH - If it's a left block and it's a MeanGreen-marked Core menu, then block display it.				
                if ( $row['position'] == $side[0] && $row['url'] == 'MeanGreen' && $row['bkey'] == 'menu')
                {
                 
// echo "\nmenu title=" . $row['title'] . " - " . $menu_title;
                    if ( $menu_title != "")
                    {
                        if ( $row['title'] == $menu_title )
                        {
                            $grn_menu_blk_chosen = true;    // Found the correct contextual menu block
                        }
                        else
                            continue;
                    }
                    ...
                    {
                        if ( $grn_menu_blk_chosen == true)   
                            continue;    // Mean green context menu block alrdy shown
                        else
                            $grn_menu_blk_chosen = true;    // Use the default main menu block (top most ordered one)
                    }
                }

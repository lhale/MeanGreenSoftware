...
function blocks_menu_block($row)
{
    ...
    // Display style
    // style = 1 - simple (bulleted) list
    // style = 2 - drop-down list
    // style = 3 - unbulleted list (can be used for narrow blocks)
...
function blocks_menu_select($row)
{
    ...
    // Defaults
    if (empty($vars['style']) || !is_numeric($vars['style'])) {
        $vars['style'] = 1;    // default bulleted list
    }
    ...
    $output .= ' /></td></tr><tr><td><label for="menu_dropdown">'._MENU_AS_UNBULLETED.'</label>:</td><td><input type="radio" id="menu_unbulleted" name="style" value="3"';
    if ($vars['style'] == 3) {
        $output .= ' checked="checked"';
    }
    $output .= ' /></td></tr>';
...
function startMenuStyle($style)
{
    ...
    else if ($style == 2) {    // drop down select box style list
...
function addMenuStyledUrl($style, $name, $url, $comment)
{
    ...
    if ($style == 1 || $style == 3) 
    {
        ...
        }  // End Bracket Linking, start normal link row:
            if ($style == 1 )
                $content = '<li><a href="'.pnVarPrepForDisplay($url).'" title="'.pnVarPrepHTMLDisplay($comment).'">'.pnVarPrepHTMLDisplay($name).'</a></li>'."\n";
            else    // NOTE: The added <br> is a bandaid 
                $content = '<a href="'.pnVarPrepForDisplay($url).'" title="'.pnVarPrepHTMLDisplay($comment).'">'.pnVarPrepHTMLDisplay($name).'</a>'."<br />\n";
        }
    } else if ($style == 2) {
        // Drop-down lilst
        if (empty($url)) {

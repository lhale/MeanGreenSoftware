<?php
// $Id: pnAPI.php,v 1.145 2006/01/27 08:56:48 larsneo Exp $
// ----------------------------------------------------------------------
...
*/
define('_PN_VERSION_NUM', '0.7.6.2');
...
function pnConfigGetVar($name, $default=null)
{
    ...
    // RNG: 0.8 compat layer 
    if (isset($GLOBALS['PNConfig']['System'][$name])) {
        $result = $GLOBALS['PNConfig']['System'][$name];
    } else {
...
function pnInit()
{
	...
	// V4B RNG
        if ($pnconfig['V4B_CONFIG_HTTP_LOGIN_ALLOW'] && !pnUserLoggedIn()) {
            pnUserLogInHTTP ($pnconfig['V4B_CONFIG_HTTP_LOGIN_FORCE_SECURITY']);
        }
	// V4B RNG
    ...
    // V4B RNG: v4blib merges backport
    ...
    // V4B RNG: v4blib merge backport end
...
function &pnDBGetTableOptions()
{
    ...
    $tableoptions = array('mysql' => 'type = ' . pnConfigGetVar('dbtabletype'));
    return $tableoptions;
}

<?php
// $Id: pnSession.php,v 1.21 2005/09/20 14:57:46 drak Exp $
// ----------------------------------------------------------------------
...
//    }
    ...
    register_shutdown_function('session_write_close');   // LDH PHP 5.2.0 fix per http://community.zikula.org/module-Forum-viewtopic-topic-50938-start-0.htm#pid217436
    // Session handlers
...
function pnSessionWrite($sessid, $vars)
{
    ...
    $query = "UPDATE $sessioninfotable
              ...
              WHERE $sessioninfocolumn[sessid] = '" . pnVarPrepForStore($sessid) . "'";
    $dbconn->Execute($query);  // LDH - see http://community.zikula.org/module-Forum-viewtopic-topic-50938-start-0.htm#pid217436 for the 'fix'
...
function pnSessionGC($maxlifetime=3000)
{
    ...
    switch (pnConfigGetVar('seclevel')) {
        ...
        case 'Medium':
            ...
            $where = "WHERE ($sessioninfocolumn[vars] NOT LIKE '%PNSVrememberme|%'
                      AND $sessioninfocolumn[lastused] < " . (time() - (pnConfigGetVar('secinactivemins') * 60)) . ")
                      AND $sessioninfocolumn[firstused] < " . (time() - (pnConfigGetVar('secmeddays') * 86400));
            break;
        case 'High':
        default:

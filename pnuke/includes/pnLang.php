<?php
// $Id: pnLang.php,v 1.20 2005/10/05 06:49:17 davidnelson Exp $
// ----------------------------------------------------------------------
...
function ml_ftime($datefmt, $timestamp = -1)
{
    ...
    $ml_date = preg_replace('%a', $day_of_week_short[(int) strftime('%w', $timestamp)], $datefmt);
    $ml_date = preg_replace('%A', $day_of_week_long[(int) strftime('%w', $timestamp)], $ml_date);
    $ml_date = preg_replace('%b', $month_short[(int) strftime('%m', $timestamp)-1], $ml_date);
    $ml_date = preg_replace('%B', $month_long[(int)strftime ('%m', $timestamp)-1], $ml_date);
    ...
    $ml_date = preg_replace('%Z', $timezone_all [$indexofzone], $ml_date);
